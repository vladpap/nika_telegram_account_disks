...
export const DiskPage = () => {
  ...
  return (
    <div className="px-5 pt-4 pb-[120px]">
      <div>
        <Header
          label1="Дисков в проекте"
          value1={43}
          label2="Дисков в работе"
          value2={2}
        />
      </div>
      ...
      <div className="mt-4 py-4 flex-row justify-center border-solid rounded-[12px] border border-tg-hint">
        <div
          onClick={() => navigate(`/disk/${disk.id}/image`)}
          className="cursor-pointermt-4 flex justify-center"
        >
          <AdaptiveImage
            src={disk.image}
            alt={disk.diskName}
        ...
        <div className="mt-4 px-4 flex">
          <div className="text-right text-base">
            ...
            <div>
              <span className="text-tg-text">Дата приёма :</span>
            </div>
            <div>
            ...
            {disk.noteIn && disk.noteIn.trim() !== "" && (
              <div>
                <span className="text-tg-text">Примечание :</span>
              </div>
            )}
          </div>
          ...
          <div className="flex-1 pl-2 font-medium">
            <div>
              <span className="text-tg-link">{disk.project?.title ?? "-"}</span>
            </div>
            <div>
              <span className="text-tg-link">{disk.date}</span>
            </div>
            <div>
              <span className="text-tg-link">{disk.receiver?.name ?? "-"}</span>
            </div>
            <div>
              <span className="text-tg-link">{disk.diskName}</span>
            </div>
            <div>
              <span className="text-tg-link">{disk.noteIn}</span>
            </div>
          </div>
        </div>
      </div>
